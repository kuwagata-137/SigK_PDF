...
起草日: 2026-09-04
ステータス: **確定・実装済み**（2026-09-04 確定・同日実装。論点4件は起草前にユーザーが決定。末尾「ユーザーの確定」を
参照。残る細部は起草者の推しで書き、その一覧を「起草者の判断で決めたもの」に置いた。仕様書から変えた点と
完了判定の結果は末尾「実装の記録」にある）
...
## 実装の記録（2026-09-04・`claude/phase-2-facing` ブランチ）

確定事項1〜31 をそのまま実装した。仕様書から変えた点・足した点は次のとおり。

| 項目 | 仕様書 | 実装 | 理由 |
|---|---|---|---|
| 切り替え時の再描画 | 配置し直す | 追従中で倍率が変わったときだけ canvas を捨てる。倍率が同じなら枠を動かすだけ | 倍率が同じなら canvas の解像度も同じで、捨てる理由が無い |
| `docs/02` の更新 | 設定キーの記述 | 変更なし | `docs/02` は設定キーの一覧を持っていなかった（`settings.js` の役目だけ書いてある） |
| 起動確認 | `SIGK_SMOKE_FACING=1` | 加えて `SIGK_SMOKE_FACING_STAY=1` を足した。画面は見開きのまま残し、覚える値だけ起動時のものへ戻す。先頭で閲覧モードへ切り替える | スクリーンショットを撮るため。前回の起動確認がツールモードで終わっていると、ページビューが隠れたまま撮ることになった |
| 1ページの文書 | `contentWidth = leftHalf` | 同じ（右に置くページが無ければ間隔を足さない） | — |

### 完了判定の結果

| # | 判定 | 結果 |
|---|---|---|
| 1 | 「見開き」で切り替わり `.active`、文書が無いと押せない | ✅ jsdom（`test/viewer-facing.test.js`）・起動確認（`active: true`） |
| 2 | 1-2, 3-4 が同じ高さ、奇数末尾は左に単独、「幅に合わせる」で2枚が収まる | ✅ `test/viewer-layout.test.js`・起動確認（40ページ・3ページとも `sameRow: true`、3ページで `lastAlone: true`、`fitMatches: true`。視野 935px で1枚 436px・器 888px・倍率 112% → 55%） |
| 3 | ◀ ▶ が2ページ動く、右ページの番号が保たれる | ✅ jsdom・起動確認（`afterNext: 2`、`goToPage(3)` のあと scroll を挟んでも `keepsRight: 3`）。実機のスクリーンショット `screenshots/phase2-facing-app.png` は 3-4 を映し、ページ番号は 4 |
| 4 | ページ編集のあとも見開きのまま | ✅ jsdom（`applyPlan` で逆順）・起動確認 `SIGK_SMOKE_PAGES=select:0,rotate,select:3,delete` と併用（39ページ・回転1・`ratiosMatch: true`。回した紙が左、次のページが右に並ぶ）。回した横向きの紙が組の中で最も広くなるため横スクロールバーが出る。`spec-1-5` の既知の限界のとおり |
...
| 項目 | 結果 |
|---|---|
| 切り替え（`setPageLayout` → 配置し直し → 描画更新まで） | 約 400ms（40ページ・3ページとも。待ち時間 400ms を含むので上限値） |
| 倍率 | 単ページ 112% → 見開き 55%（`fitWidthZoom` の計算値と一致） |
| 描いている枚数（現在 3ページ目・見開き） | 6枚（`[0..5]`。可視2行＋前後2枚） |
| 編集（回転1・削除1）→ 並び直し | `applyPlan` 5.3ms |
| 配布物（`npm run dist` → `dist/win-unpacked`） | `SIGK_SMOKE=1` は `problems: []`。`SIGK_SMOKE_FACING=1`（3ページ）は開発ツリーと同じ結果（`sameRow`・`lastAlone`・`fitMatches`・`afterNext: 2`・`saved: 'facing'`・`restoredLayout: 'single'`、切り替え 約 410ms）。インストーラー `SigK PDF Setup 0.1.0.exe` は 112.9MB |
