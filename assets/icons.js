(function (root) {
  ...
  const ICONS = {
    ...
    // 画像→PDF（spec-3-1 確定事項35）。左に画像（山と太陽の入った枠）、
    // 右に紙、あいだに右向きの矢印。結合・分割と同じ線だけの造りにする。
    convert: [
      ['rect', { x: 3, y: 6, width: 8, height: 8, rx: 1 }],
      ['path', { d: 'M3 12l2.5-2.5L8 12l1.5-1.5L11 12' }],
      ['circle', { cx: 8.5, cy: 8.5, r: 0.9 }],
      ['path', { d: 'M15 6h6v12h-6z' }],
      ['path', { d: 'M5 18h6m0 0l-2-2m2 2l-2 2' }],
    ],
    // PDF→画像（spec-3-3 確定事項29）。convert の左右を入れ替えた構図。左に紙、右に画像の枠。
    toImage: [
      ['path', { d: 'M3 6h6v12H3z' }],
      ['rect', { x: 13, y: 6, width: 8, height: 8, rx: 1 }],
      ['path', { d: 'M13 12l2.5-2.5L18 12l1.5-1.5L21 12' }],
      ['circle', { cx: 18.5, cy: 8.5, r: 0.9 }],
      ['path', { d: 'M13 18h6m0 0l-2-2m2 2l-2 2' }],
    ],
    // 行を掴むつまみ（結合画面の並べ替え）。
    grip: [
